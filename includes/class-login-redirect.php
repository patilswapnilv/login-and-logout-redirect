...
class Login_Redirect
{
    ...
    function __construct()
    {
        if (!isset($_REQUEST['redirect_to']) || $_REQUEST['redirect_to'] == admin_url()) {
            ...
            add_filter('woocommerce_login_redirect', array(&$this, 'redirect_woocommerce'), 9999, 2);
        }
    ...
    function redirect($redirect_to, $requested_redirect_to, $user)
    {
        ...
        if (!is_wp_error($user) && !$reauth && !$interim_login) {
            $custom_redirect = $this->is_plugin_active_for_network(plugin_basename(__FILE__))
                ? get_site_option('login_redirect_url')
                : get_option('login_redirect_url');
                
            if (!empty($custom_redirect)) $redirect_to = $custom_redirect;
        }
    ...
    /**
     * Redirect user on login with WooCommerce
     * */
    function redirect_woocommerce($redirect_to, $user)
    {
        return $this->redirect($redirect_to, false, $user);
    }
